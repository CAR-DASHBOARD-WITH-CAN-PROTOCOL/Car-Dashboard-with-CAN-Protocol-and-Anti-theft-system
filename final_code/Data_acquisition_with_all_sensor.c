...
In Can1> SET PRECSCALER  24
BIT SEGMENT 1> 13
BIT SEGMENT 2> 2
Sjw> 1
Activate Can interrup from Nvic parameters TX for trasmitter node and RX for receiver node
...
int main(void)
{
  ...
  TxHeader.DLC = 3; // Data length code, updated to 3 as we are sending 3 bytes
  ...
  while (1)
  {
    ...
    uint8_t new_proximity_sensor = (HAL_GPIO_ReadPin(GPIOA, GPIO_PIN_1) == GPIO_PIN_SET) ? 1 : 0;
    if (new_proximity_sensor != proximity_sensor)
    {
      proximity_sensor = new_proximity_sensor;
      HAL_GPIO_WritePin(GPIOD, GPIO_PIN_13, (proximity_sensor == 1) ? GPIO_PIN_SET : GPIO_PIN_RESET); // Debug LED
    }
    if(DHT11_Start())
          {
...
/* USER CODE END MX_GPIO_Init_1 */
  ...
  HAL_GPIO_WritePin(GPIOD, GPIO_PIN_12|GPIO_PIN_13|GPIO_PIN_14|GPIO_PIN_15
                          |Temperature_sensor_Pin, GPIO_PIN_RESET);

  /*Configure GPIO pins : Fingerprint_sensor_Pin Proximity_sensor_Pin */
  GPIO_InitStruct.Pin = Fingerprint_sensor_Pin|Proximity_sensor_Pin;
